...
把这个项目写在简历上时，建议突出以下关键词：

> **项目：CodeCritic — 多智能体代码评审辩论系统**
>
> - 基于 **LangChain + LangGraph** 构建多智能体协作框架，5 个专业 Agent 从安全、性能、风格、正确性、架构维度并行审查代码
> - 设计**辩论仲裁机制**，Judge Agent 自动检测分歧，触发多轮 Agent 辩论后由 Arbiter 仲裁，提升评审准确率
> - 集成**持久化记忆系统**，基于 SQLite 实现审查历史记忆，同一会话内跨次审查自动继承上下文
> - 实现 **KV Cache 预热**策略，同模型组 Agent 共享代码前缀缓存，显著减少冗余 Token 消耗
> - 支持**自定义 Agent Prompt** 的 YAML 配置系统，可动态加载/卸载 Agent，每个 Agent 可指定不同模型
> - 提供 **Web UI**（GPT 风格对话界面）与 CLI 双模式，支持对话管理、模型切换、实时轮询
> - 通过 **Pydantic 严格 Schema** 约束 Agent 输出，确保下游可解析可处理
